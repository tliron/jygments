...
public class Css2Lexer extends RegexLexer
{
	public Css2Lexer()
	{
		...
		rule( "basics", "\\s+", "Text" );
		rule( "basics", "/\\*(?:.|\\n)*?\\*/", "Comment" );
		rule( "basics", "\\{", "Punctuation", "content" );
		rule( "basics", "\\:[a-zA-Z0-9_-]+", "Name.Decorator" );
		rule( "basics", "\\.[a-zA-Z0-9_-]+", "Name.Class" );
		rule( "basics", "\\#[a-zA-Z0-9_-]+", "Name.Function" );
		rule( "basics", "@[a-zA-Z0-9_-]+", "Keyword", "atrule" );
		rule( "basics", "[a-zA-Z0-9_-]+", "Name.Tag" );
		rule( "basics", "[~\\^\\*!%&\\[\\]\\(\\)<>\\|+=@:;,./?-]", "Operator" );
		rule( "basics", "\"(\\\\\\\\|\\\\\"|[^\"])*\"", "String.Double" );
		rule( "basics", "'(\\\\\\\\|\\\\'|[^'])*'\"", "String.Single" );

		rule( "atrule", "\\{", "Punctuation", "atcontent" );
		rule( "atrule", ";", "Punctuation", "#pop" );
		include( "atrule", "basics" );

		include( "atcontent", "basics" );
		rule( "atcontent", "}", "Punctuation", "#pop:2" );

		rule( "content", "\\s+", "Text" );
		rule( "content", "}", "Punctuation", "#pop" );
		rule( "content", "url\\(.*?\\)", "String.Other" );
		rule( "content", "^@.*?$", "Comment.Preproc" );
		...
		rule( "content", "(azimuth|background-attachment|background-color|" + "background-image|background-position|background-repeat|" + "background|border-bottom-color|border-bottom-style|"
			...
			+ "upper-alpha|upper-latin|upper-roman|uppercase|url|" + "visible|w-resize|wait|wider|x-fast|x-high|x-large|x-loud|" + "x-low|x-small|x-soft|xx-large|xx-small|yes)\\b", "Keyword" );
		...
		rule( "content", "\\!important", "Comment.Preproc" );
		rule( "content", "/\\*(?:.|\\n)*?\\*/'", "Comment" );
		rule( "content", "\\#[a-zA-Z0-9]{1,6}", "Number" );
		rule( "content", "[\\.-]?[0-9]*[\\.]?[0-9]+(em|px|\\%|pt|pc|in|mm|cm|ex)", "Number" );
		rule( "content", "-?[0-9]+", "Number" );
		rule( "content", "[~\\^\\*!%&<>\\|+=@:,./?-]+", "Operator" );
		rule( "content", "[\\[\\]();]+", "Punctuation" );
		rule( "content", "\"(\\\\\\\\|\\\\\"|[^\"])*\"", "String.Double" );
		rule( "content", "\"'(\\\\\\\\|\\\\'|[^'])*'", "String.Single" );
		rule( "content", "[a-zA-Z][a-zA-Z0-9]+", "Name" );
